...
@dataclass
class GitLabFormLogs:
    """Container for logs intercepted from the GitLabForm CLI UI."""
...
@pytest.fixture
def gitlabform_logs(monkeypatch) -> Generator[GitLabFormLogs, None, None]:
    """
    Intercepts and stores logs from GitLabForm's CLI UI (debug, info, error).
    
    WHEN TO USE THIS FIXTURE:
    - Use this for messages generated by 'cli_ui' (e.g., verbose() calls in 
      processors, progress updates, and user-facing CLI output).
    - Use this when standard fixtures (capsys/caplog) return empty strings 
      despite the output being visible in the terminal.

    WHEN TO USE 'caplog':
    - Use 'caplog' for standard Python logging records (e.g., logs from 
      the 'logging' module, or underlying libraries like 'urllib3').
    
    WHY THIS IS NEEDED:
    GitLabForm uses 'cli_ui' for its primary output. Because 'cli_ui' can 
    be configured to bypass standard Python logging handlers, pytest's 
    'caplog' fixture often cannot "see" these messages. This fixture 
    patches the 'cli_ui._print' method directly at the source, which 
    ensures we capture messages even if a processor has already aliased 
    the debug function (e.g., 'from cli_ui import debug as verbose').
    """
    logs = GitLabFormLogs()

    def _capture(level_list, *args):
        msg = " ".join([str(arg) for arg in args if not str(arg).startswith("Color")])
        msg = msg.strip()
        if msg:
            level_list.append(msg)
            print(f"{msg}")
    ...
    # We look for all currently loaded modules that might have an aliased 'verbose'
    # This solves the "from cli_ui import debug as verbose" namespace issue 
    # without needing markers for every test.
    for name, module in list(sys.modules.items()):
        if name.startswith("gitlabform.processors."):
