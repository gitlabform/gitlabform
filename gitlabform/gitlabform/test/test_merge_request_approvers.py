...
from gitlabform.gitlabform.test import create_group, create_project_in_group, create_users_in_project, \
    add_users_to_group, get_gitlab, GROUP_NAME, OWNER_ACCESS, remove_users_from_group
...
def gitlab(request):
    ...
    add_users_to_group(GROUP_WITH_USER1_AND_USER2, ['merge_requests_user1', 'merge_requests_user2'], OWNER_ACCESS)
...
class TestMergeRequestApprovers:
    ...
    def test__more_than_one_user(self, gitlab):
        gf = GitLabForm(config_string=config__approvers_more_than_one_user,
    ...
    def test__more_than_one_user_and_more_than_one_group(self, gitlab):
        gf = GitLabForm(config_string=config__approvers_more_than_one_user_and_more_than_one_group,
                        project_or_group=GROUP_AND_PROJECT_NAME)
        gf.main()
        ...
        assert groupnames_in_rule == {'gitlabform_tests_group_with_user3', 'gitlabform_tests_group_with_user4'}

        print(str(rule['eligible_approvers']))
        assert len(rule['eligible_approvers']) == 4
        eligibleapprovers_in_rule = set([user['username'] for user in rule['eligible_approvers']])
        assert eligibleapprovers_in_rule == {'merge_requests_user1', 'merge_requests_user2',
