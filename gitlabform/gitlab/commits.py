...
class GitLabCommits(GitLabCore):
    ...
    def get_last_commit_attributes(self, project_and_group_name, branch):
        branch = self._make_requests_to_api("projects/%s/repository/branches/%s", (project_and_group_name, branch))

        last_commit_hash = branch['commit']['id']

        commit = self._make_requests_to_api("projects/%s/repository/commits/%s", (project_and_group_name, last_commit_hash))
