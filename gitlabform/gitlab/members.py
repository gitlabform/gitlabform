...
class GitLabMembers(GitLabCore):
    ...
    def update_member_of_project(self, group_and_project_name, user, access_level, expires_at):
        ...
        if access_level is not None:
            data['access_level'] = access_level
        logging.warning("\t~~ UPD user access: %s [%s] to: %s", user, access_level, group_and_project_name)
    ...
    def update_member_of_group(self, group_name, user, access_level, expires_at):
        ...
        if access_level is not None:
            data['access_level'] = access_level
        logging.warning("\t~~ UPD user membership to group: %s [%s] to: %s", user, access_level, group_name)
    ...
    # def update_group_share_of_project(self, group_name, user, access_level, expires_at):
    #     user_id =self._get_user_id(user)
    #     data = {
    #         "user_id": user_id,
    #         "expires_at": expires_at
